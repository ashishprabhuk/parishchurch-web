...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-heading text-3xl">Content management</h1>
          ...
          <Link key={entity.type} to={`/admin/${entity.type}`}>
            <Card className="group hover:border-primary/60 h-full transition-colors">
              <CardContent className="flex h-full flex-col p-5">
                <h2 className="font-heading text-xl">{entity.label}</h2>
                <p className="text-muted-foreground mt-1.5 flex-1 text-sm">
                  {entity.description}
                </p>
                <span className="text-primary mt-4 inline-flex items-center gap-1.5 text-xs font-semibold tracking-[0.12em] uppercase">
                  Manage
